...
/**
 * Constructs a new execution configration dialog.
 */
var ExecutionConfigrationDialog = function(editorUi)
{
    var graph = editorUi.editor.graph;
    ...
    var runConfiguration = document.createElement('div');
    runConfiguration.innerHTML =  "<label>Run configuration: </label></label><select>"+
		"<option value='aaa'>aaa</option>"+
		"<option value='bbb'>bbb</option>"+
		"<option value='ccc'>ccc</option>"+
		+"</select>";
    div.appendChild(runConfiguration);
...
(function()
{
	...
	mxGraphView.prototype.validateBackgroundStyles = function()
	{
		...
		if (graph.view.backgroundPageShape != null)
		{
			...
			graph.view.backgroundPageShape.node.style.backgroundImage = 'none';     //修改编辑器区域的背景格子，默认没有背�?			graph.view.backgroundPageShape.node.style.backgroundColor = color;
		...
		var drawPageBreaks = mxUtils.bind(this, function(breaks)
		{
			if (breaks != null)
			{
				...
				for (var i = 0; i <= count; i++)
				{
					...
					{
                        this.pageBreakColor="#ffffff";//设置多块画布间的边线颜色为白色
						var pageBreak = new mxPolyline(pts, this.pageBreakColor);
						pageBreak.dialect = this.dialect;
						pageBreak.isDashed = this.pageBreakDashed;
						pageBreak.pointerEvents = false;
                        pageBreak.strokewidth=0;//设置多块画布间的边线宽度为0
