...
EditorUi = function(editor, container, lightbox)
{
    ...
    //在chromless模式并且未编辑状态下，禁用graph并强制平移
    ...
    if (this.container == document.body)
    {
        ...
        this.resultContainer.onselectstart = textEditing;
        this.resultContainer.onmousedown = textEditing;
 ...
 */
EditorUi.prototype.resultHeight = 250;
...
EditorUi.prototype.vsplitPosition = (screen.height <= 480) ? 130 : 250;
...
//获取url中的参数
EditorUi.prototype.getUrlParam= function(name)
{
...
EditorUi.prototype.init = function()
{
    ...
    var curType=this.getUrlParam("type");
    if(curType=='job'){
        this.currentMode_Is_Job=true;
    }else {
        this.currentMode_Is_Job=false;
    }

    this.fileName=this.getUrlParam("name");
...
EditorUi.prototype.controlResultpanel =function(){
    this.vsplit.triggerClick=true;
    $(".geVsplit").click();
};
...
EditorUi.prototype.refresh = function(sizeDidChange)
{
    ...
    var effVsplitPosition = Math.max(0, Math.min(h-this.vsplitPosition-12, h - this.splitSize));
    ...
    this.hsplit.style.bottom =  '0px';
    this.hsplit.style.left = effHsplitPosition + 'px';

    this.diagramContainer.style.left = (this.hsplit.parentNode != null) ? (effHsplitPosition + this.splitSize) + 'px' : '0px';
    this.diagramContainer.style.top = this.sidebarContainer.style.top;
    this.diagramContainer.style.bottom =(h-effVsplitPosition)  + 'px';
    this.diagramContainer.style.right = '0px';

    this.vsplit.style.bottom =  (h-effVsplitPosition-this.splitSize)  + 'px';
    this.vsplit.style.left = effHsplitPosition + 'px';
    this.vsplit.style.right = '0px';
    this.vsplit.triggerClick=false;

    this.resultContainer.style.left = this.diagramContainer.style.left;
    this.resultContainer.style.right = '0px';
    this.resultContainer.style.bottom = '0px';
    this.resultContainer.style.height=(h-effVsplitPosition-this.splitSize)  + 'px';
    //this.resultContainer.style.display = (this.resultbar != null) ? '' : 'none';
  ...
  /*  if (this.tabContainer != null)
    ...
    if (this.tabContainer != null)
    {
        this.tabContainer.style.bottom = (this.resultHeight + off) + 'px';
...
EditorUi.prototype.createDivs = function()
{
    ...
    this.resultContainer = this.createDiv('geFooterContainer');
    ...
    this.resultContainer.style.left = '220px';
    this.resultContainer.style.right = '0px';
...
/**
 * Creates a new resultbar for the given container.
 */
EditorUi.prototype.createResultbar = function(container)
{
    return new Resultbar(this, container);
};
...
{
    var start = null;//开始的位置
    var initial = null;//移动前的初始位置
    var ignoreClick = true;//标识是否停止移动
    var last = null;
...
/**
 * Saves the current graph and kettle xml under the given filename.
 */
EditorUi.prototype.saveFile = function(name)
{
    if (name != null)
    {
        ...
        var graphXml = mxUtils.getXml(this.editor.getGraphXml());
        var kettleXml = this.getKettleXml();
        ...
        try
        {
            if (Editor.useLocalStorage)
            {
                ...
                localStorage.setItem(name, graphXml);
                this.editor.setStatus(mxUtils.htmlEntities(mxResources.get('saved')) + ' ' + new Date());
            }
            else
            {
                if (graphXml.length < MAX_REQUEST_SIZE)
                {
                    var paramJsonObj = new Object();
                    if(this.currentMode_Is_Job){  //全局变量判断当前执行文件的类型
                        paramJsonObj.executeObjectType = "job";
                    }else{
                        paramJsonObj.executeObjectType = "transformation";
                    }
                    paramJsonObj.fileName = this.fileName;
                    paramJsonObj.graphXml = graphXml;
                    paramJsonObj.kettleXml = kettleXml;

                    var last_gritter;
                    var callback = {
                        onSuccess:function(paramJsonObj){
                            if (last_gritter) $.gritter.remove(last_gritter);
                            if(paramJsonObj.errorCode == "0"){
...
EditorUi.prototype.getKettleXml = function(name)
{
...
EditorUi.prototype.destroy = function()
{
    ...
    var allContainer = [this.menubarContainer, this.toolbarContainer, this.sidebarContainer,
        this.resultContainer, this.diagramContainer,
        this.chromelessToolbar, this.hsplit,this.vsplit, this.layersDialog];

    for (var i = 0; i < allContainer.length; i++)
    {
        if (allContainer[i] != null && allContainer[i].parentNode != null)
        {
            allContainer[i].parentNode.removeChild(allContainer[i]);
        }
    }
};
