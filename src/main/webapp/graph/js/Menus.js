 ...
 * Sets the default font size.
 */
Menus.prototype.defaultMenuItems = ['file', 'edit', 'view', 'execution', 'help'];
...
Menus.prototype.init = function () {
    ...
    this.put('view', new Menu(mxUtils.bind(this, function (menu, parent) {
        this.addMenuItems(menu, ((this.editorUi.format != null) ? ['formatPanel'] : []).concat(['zoomIn', 'zoomOut', 'resetView', '-', 'resultPanel'], parent));
    })));
    ...
    this.put('file', new Menu(mxUtils.bind(this, function (menu, parent) {
        this.addMenuItems(menu, ['save','print'], parent);
    })));
    this.put('edit', new Menu(mxUtils.bind(this, function (menu, parent) {
        this.addMenuItems(menu, ['undo', 'redo']);
    })));
    this.put('execution', new Menu(mxUtils.bind(this, function (menu, parent) {
        this.addMenuItems(menu, ['run', 'runOption'], parent);
    }))).isEnabled = isGraphEnabled;
    this.put('help', new Menu(mxUtils.bind(this, function (menu, parent) {
        this.addMenuItems(menu, ['document']);
    })));
};
...
Menus.prototype.createPopupMenu = function (menu, cell, evt) {
    ...
    if (graph.isSelectionEmpty()) {
        this.addMenuItems(menu, ['undo', 'redo', 'pasteHere', '-', 'selectAll'], null, evt);
        return;
    }

    this.addMenuItems(menu, ['cut', 'copy', 'duplicate', 'delete'], null, evt);
    cell = graph.getSelectionCell();
    var state = graph.view.getState(cell);

    if (state != null) {
        /* 组合/取消组合，目前不用，可能以后会用到，暂时注释
        ...
        if (graph.getSelectionCount() == 1 && graph.getModel().isVertex(cell)) {
            this.addMenuItems(menu, ['-', 'editDetails'], null, evt);
        }
    }
