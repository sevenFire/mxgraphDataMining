...
* 生成步骤细节对话框
* */
var EditDetailsDialog = function(ui, cell) {
	var value = cell.getValue();
    var $div = $(cell.getLocalHtml());
    this.initData($div, value);
    ...
    var applyBtn = mxUtils.button(mxResources.get('apply'), function()
    {
        try
        {
            ...
            $div.find("[data-attribute]").each(function(){
            	var attr = $(this).data("attribute");
            	value.setAttribute(attr, $(this).val());
			});
            cell.setValue(value);
        }
        catch (e)
...
* 初始化数据
*
* 当 dom 对象存在 data-oninit 属性时，执行属性对应的方法
* 否则，存在 data-attribute 属性时，执行默认赋值方法
*
* */
EditDetailsDialog.prototype.initData = function($div, value){
	var edd = this;
	$div.find("[data-attribute], [data-oninit]").each(function(){
		var $this = $(this);
        var initfunc;
		if(initfunc = $this.data("oninit")){
            edd.initFuncs[initfunc]($this, value);
            return;
		}
		edd.initFuncs.defaultFunc($this, value);
	})
};

/*
* 初始化方法对象
* 默认参数为 $this：dom 的 jquery 对象
*          value: cell 的 value 值
* */
EditDetailsDialog.prototype.initFuncs = {
	//默认赋值方法
	defaultFunc: function($this, value){
		var attr = $this.data("attribute");
		var v = value.getAttribute(attr);
		if(v != null) {
			$this.val(v);
		};
	},
	...
	// 获取数据库信息
	addDBList: function($this, value){
		//TODO this value is temporary
		$this.append($('<option value="1">111</option><option value="2">222</option>'));
		this.defaultFunc($this, value);
	},
}
