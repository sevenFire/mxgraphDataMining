...
Actions.prototype.init = function()
{
	...
	var fileName = "111";
	this.addAction('save', function() { ui.saveFile(fileName); }, null, null, Editor.ctrlKey + '+S').isEnabled = isGraphEnabled;
	...
	this.addAction('print...', function() { ui.showDialog(new PrintDialog(ui).container, 320, 200, true, true); }, null, 'sprite-print', Editor.ctrlKey + '+P');
	this.addAction('preview', function() { mxUtils.show(graph, null, 10, 10); });
	
	// Edit actions
    this.addAction('run', function() { ui.showDialog(new ExecutionConfigrationDialog(ui).container,400,220, true, true); }, null, 'sprite-run', Editor.ctrlKey + '+R');
    this.addAction('stop', function() { ui.undo(); }, null, 'sprite-stop', Editor.ctrlKey + '+F3');
    ...
    this.addAction('resultpanel', function() { ui.controlResultpanel(); }, null, 'sprite-resultpanel', 'null');
	...
	this.addAction("editDetails", function(){
        var cell = graph.getSelectionCell() || graph.getModel().getRoot();

        if (cell != null)
        {
            var dlg = new EditDetailsDialog(ui, cell);
            ui.showDialog(dlg.container, 620, 500, true, false, null, false);
        }
    ...
    // Execution actions
    this.addAction('runOption', function(){ ui.showDialog(new ExecutionConfigrationDialog(ui).container,400,220, true, true); }, null, 'sprite-run', Editor.ctrlKey + '+F10');

    //Tools actions
    this.addAction('database', function()
    {
        graph.connectionHandler.setCreateTarget(!graph.connectionHandler.isCreateTarget());
        // ui.fireEvent(new mxEventObject('copyConnectChanged'));
    });
    this.addAction('resourceLibrary', function()
    {
        graph.connectionHandler.setCreateTarget(!graph.connectionHandler.isCreateTarget());
        // ui.fireEvent(new mxEventObject('copyConnectChanged'));
    });
	...
	// Help actions
	this.addAction('document', function()
	{
		var ext = '';
