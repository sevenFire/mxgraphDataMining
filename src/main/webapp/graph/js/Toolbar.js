...
Toolbar.prototype.init = function()
{
	...
	//添加执行和暂停按钮
    this.addSeparator();
    var elts = this.addItems(['run','stop']);
    elts[0].setAttribute('title', mxResources.get('run') + ' (' + this.editorUi.actions.get('run').shortcut + ')');
    elts[1].setAttribute('title', mxResources.get('stop') + ' (' + this.editorUi.actions.get('stop').shortcut + ')');
	...
	if (sw >= 320)
	{
		var elts = this.addItems(['-', 'delete','-','resultpanel']);
		elts[1].setAttribute('title', mxResources.get('delete') + ' (' + this.editorUi.actions.get('delete').shortcut + ')');
		elts[3].setAttribute('title', mxResources.get('resultpanel') + ' (' + this.editorUi.actions.get('resultpanel').shortcut + ')');
	}
